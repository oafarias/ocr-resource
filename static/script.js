...
async function executeOCR() {
    ...
    // 1. Captura os inputs de texto
    const endpointInput = document.getElementById("endpoint").value.trim();
    const visionKey = document.getElementById("chave").value.trim();

    // 2. CAPTURA DO ARQUIVO (Faltou essa linha!)
    const file = fileInput.files[0]; 

    if (!endpointInput || !visionKey) {
        addLog("🚫 Por favor, preencha todos os campos de configuração.", 'error');
        return;
    }

    // Valida se o arquivo foi selecionado antes de prosseguir
    if (!file) {
        addLog("🚫 Por favor, selecione uma imagem primeiro.", 'error');
        return;
    }

    let baseAddr = endpointInput;
    if (!baseAddr.startsWith('http')) {
    ...
    const url = `${baseAddr}vision/v3.2/read/analyze`;
    console.log("Enviando para: ", url);

    try {
        // Agora o file.name vai funcionar!
